...
The first pass at this just isolated agents with worktrees and routed
everything through PRs + a `agent-merge-queue` label. That leans entirely
on GitHub's merge queue feature actually being enabled and working — which
is a real assumption to make if a prior attempt already failed once.
...
1. Drop `AGENTS.md` (below) into the repo root, alongside `SOUL.md`.
2. Tell most agent sessions nothing extra — they default to **Worker**.
3. For the session that should process the queue, say explicitly:
   `"Run as Merge Agent on this repo."`
4. Make sure `.github/workflows/ci.yml` has a `concurrency:` block with
   `cancel-in-progress` — the Merge Agent will check for this and add it if
   missing.

## Honest notes

This was built by researching your public posts, the `loop-library`
AGENTS.md conventions, and Peter Steinberger's worktree-based approach,
then stress-testing the draft against a separate review pass that caught
real bugs (a duplicate timestamp call, a broken polling reference, an
unquoted variable) before this version. I can't claim it's been run live
